...
import getSingleHero from "../controllers/GetSIngleHero";
...
export default function EditHero() {
  ...
  const handleOnSubmit = async (e) => {
    ...
    try {
      ...
      // 3. Send request
      const response = await fetch(`${apiBase}/api/update-hero/${id}`, {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  useEffect(() => {
    getSingleHero(setLoading, setErrors, id, setHero, apiBase);
  }, []);
  ...
  useEffect(() => {
    if (hero && hero.id) {
      setFormData({
        ...
        text: hero.text || "",
        img: null, // important: keep null unless user uploads new file
      ...
      if (hero.img) {
        setPreview(`${apiBase}/storage/heros/${hero.img}`);
      }
    }
  }, [hero]);
  ...
  return (
    <>
      <meta charSet="utf-8" />
      <title>Edit Hero - Sika's Clothing</title>
