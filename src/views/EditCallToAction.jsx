...
import getSingleCallToAction from "../controllers/GetSingleCallToAction";
...
export default function EditCallToAction() {
  ...
  const HandleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/update-single-cta/${id}`, {
        method: "POST",
        credentials: "include",
        body: payload,
        headers: {
  ...
  useEffect(() => {
    getSingleCallToAction(setLoading, setErrors, id, setCta, apiBase);
  }, []);
  ...
  useEffect(() => {
    if (cta && cta.id) {
      setFormData({
        ...
        btn_url: cta.btn_url || "",
        btn_text: cta.btn_text || "",
        bg_image: null,
      });

      if (cta.bg_image) {
        setPreviewImage(`${apiBase}/storage/call_to_action/${cta.bg_image}`);
      }
    }
  }, [cta]);
  ...
  return (
    <>
      <title>Edit Call to action - Sika's Clothing</title>
